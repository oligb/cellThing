...
namespace DarkTonic.MasterAudio {
                ...
                for (var i = 0; i < parentGroup.childCount; i++) {
                    ...
                    for (var j = 0; j < weight; j++) {
                        if (j > 0) {
                            ...
                            var resFileName = AudioResourceOptimizer.GetLocalizedFileName(variation.useLocalization,
        ...
        public static Transform CreateNewSoundGroup(DynamicSoundGroup aGroup, string creatorObjectName,
            ...
            for (var i = 0; i < aGroup.groupVariations.Count; i++) {
                ...
                for (var j = 0; j < aVariation.weight; j++) {
                    ...
                    var newVariation =
                        (GameObject)
                            GameObject.Instantiate(aVariation.gameObject, groupTrans.position, Quaternion.identity);
    ...
    public void RouteBusToUnityMixerGroup(string busName, AudioMixerGroup mixerGroup) {
...
#if UNITY_EDITOR
            var ma = Resources.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/MasterAudio.prefab",
                typeof(GameObject));
#else
        var ma = Resources.Load(MasterAudioFolderPath + "/Prefabs/MasterAudio.prefab", typeof(GameObject));
            ...
            if (ma == null) {
                Debug.LogError(
                    "Could not find MasterAudio prefab. Please drag it into the scene yourself. It is located under MasterAudio/Prefabs.");
                return null;
            }
...
#if UNITY_EDITOR
            var pc = Resources.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/PlaylistController.prefab",
                typeof(GameObject));
#else
        var pc = Resources.Load(MasterAudioFolderPath + "/Prefabs/PlaylistController.prefab", typeof(GameObject));
            ...
            if (pc == null) {
                Debug.LogError(
                    "Could not find PlaylistController prefab. Please drag it into the scene yourself. It is located under MasterAudio/Prefabs.");
                return null;
            }
...
#if UNITY_EDITOR
            var pc = Resources.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/DynamicSoundGroupCreator.prefab",
                typeof(GameObject));
#else
        var pc = Resources.Load(MasterAudioFolderPath + "/Prefabs/DynamicSoundGroupCreator.prefab", typeof(GameObject));
            ...
            if (pc == null) {
                Debug.LogError(
                    "Could not find DynamicSoundGroupCreator prefab. Please drag it into the scene yourself. It is located under MasterAudio/Prefabs.");
                return null;
            }
            // ReSharper disable once ArrangeStaticMemberQualifier
            var go = GameObject.Instantiate(pc) as GameObject;
...
#if UNITY_EDITOR
            var pc = Resources.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/SoundGroupOrganizer.prefab",
                typeof(GameObject));
#else
        var pc = Resources.Load(MasterAudioFolderPath + "/Prefabs/SoundGroupOrganizer.prefab", typeof(GameObject));
            ...
            if (pc == null) {
                Debug.LogError(
                    "Could not find SoundGroupOrganizer prefab. Please drag it into the scene yourself. It is located under MasterAudio/Prefabs.");
                return null;
            }
            // ReSharper disable once ArrangeStaticMemberQualifier
            var go = GameObject.Instantiate(pc) as GameObject;
