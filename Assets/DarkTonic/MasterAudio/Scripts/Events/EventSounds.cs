...
, IPointerDownHandler
...
#endif
 {
    ...
    // no mouse events!
        ...
        private IEnumerator TryPlayStartSound(AudioEventGroup grp, AudioEvent aEvent) {
            for (var i = 0; i < 3; i++) {
                if (MasterAudio.IgnoreTimeScale) {
                    yield return
                        StartCoroutine(CoroutineHelper.WaitForActualSeconds(MasterAudio.InnerLoopCheckInterval));
                } else {
        ...
        private PlaySoundResult PerformSingleAction(AudioEventGroup grp, AudioEvent aEvent, EventType eType,
            bool isFirstTry = true) {
            if (disableSounds || MasterAudio.AppIsShuttingDown || MasterAudio.SafeInstance == null) {
                return null;
            }

            var useSliderValue = (eType == EventType.UnitySliderChanged &&
                                  aEvent.targetVolMode == AudioEvent.TargetVolumeMode.UseSliderValue);
    ...
    // no mouse events!
                ...
                case MasterAudio.EventSoundFunctionType.PlaylistControl:
                    ...
                    if (MasterAudio.PlaylistCommandsThatFailOnStart.Contains(aEvent.currentPlaylistCommand) &&
                        eType == EventType.OnStart && isFirstTry) {
                        StartCoroutine(TryPlayStartSound(grp, aEvent));
                    } else {
